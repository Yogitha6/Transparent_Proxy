...
string getResponseFromWebServer(string requestHeaders)
{
...
void prefetchtheLinks(string link,string requestHeaders)
{
    //cout<<"Link is "<<link<<endl;
    ...
    responseContent = getResponseFromWebServer(requestHeaders);
...
void *handleRequest(void *clientSocket)
{
    ...
    if(responseContent == "")
    {
        ...
        if(numberofLinks!= 0)
        {
            ...
            {
                prefetchtheLinks(links[i],requestHeaders);
            }
        }
    }
